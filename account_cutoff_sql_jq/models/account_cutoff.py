...
class AccountCutoff(models.Model):
    ...
    def _prepare_provision_line(self, cutoff_line):
        ...
        result['account_move_ref'] = cutoff_line.origin_move_id.name
        result['account_move_label'] = cutoff_line.origin_move_line_id.display_name if not \
    ...
    def _prepare_move(self, to_provision):
        ...
        for dict in to_provision:
            amount = dict['amount']
            operating_unit_id = dict.get('operating_unit_id', False)
        ...
        res = {
            ...
            # 'to_be_reversed': True, # FIXME: Need this?
            'line_ids': movelines_to_create,
        }
        return res

    # [SQL] Not needed for now
    ...
    def create_move(self):
        ...
        if self.env.user.company_id.perform_posting_by_line_jq: # [JobQueue]
    ...
    def create_move_job_queue(self, vals):
        ...
        try:
            ...
            move_id = acc_move.id
            if self.company_id.post_cutoff_move:
                acc_move._post(soft=False)
            self.write({'move_id': move_id, 'state': 'done'})
        except Exception as e:
            raise FailedJobError(
                _("The details of the error:'%s'") % (str(e)))
