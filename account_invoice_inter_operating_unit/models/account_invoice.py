...
class AccountInvoice(models.Model):
        ...
        for src_invoice in self:
            ...
            if src_invoice.operating_unit_id == dest_ou.id:
                raise UserError(_(
                    ...
                    "and to"))
            if dest_ou and not src_invoice.auto_generated:
                MAP_INVOICE_TYPE = {
                ...
                src_invoice.sudo().\
                    with_context(force_company=dest_ou.company_id.id).\
                    _inter_ou_create_invoice(dest_ou,
                                                  dest_inv_type,
                                                  dest_journal_type)
        return res
    ...
    def _get_user_domain(self, dest_ou):
        ...
        return [
            ...
            ('operating_unit_ids', 'in', dest_ou.id),
    ...
    def _inter_ou_create_invoice(
        ...
        self._check_intercompany_product(dest_ou)
        ...
        dest_invoice_data = self._prepare_invoice_data(
            dest_inv_type, dest_journal_type, dest_ou)
        if force_number:
            dest_invoice_data['move_name'] = force_number
        dest_invoice = self.create(dest_invoice_data)
        # create invoice lines
        src_ou_partner_id = self.operating_unit_id.partner_id
        for src_line in self.invoice_line_ids:
            ...
            dest_inv_line_data = self._prepare_invoice_line_data(
                dest_invoice, dest_inv_type, dest_ou, src_line,
                src_ou_partner_id)
        ...
        if (dest_ou.invoice_auto_validation and
    ...
    def _prepare_invoice_data(self, dest_inv_type, dest_journal_type,
        ...
        if not dest_journal:
            raise UserError(_(
                ...
                % (dest_journal_type, dest_ou.company_id.name, dest_ou.company_id.id))
        # find account, payment term, fiscal position, bank.
        dest_partner_data = {
            'type': dest_inv_type,
            'partner_id': self.operating_unit_id.partner_id.id,
            'company_id': dest_ou.company_id.id,
            'operating_unit_id': dest_ou.id
        }
        ...
        return {
            'name': self.name,
            'origin': _('%s - Invoice: %s') % (self.operating_unit_id.name,
                                               self.number),
            'type': dest_inv_type,
    ...
    def _prepare_invoice_line_data(self, dest_invoice, dest_inv_type,
        ...
        dest_line_data = {
            ...
            'name': '',
            'partner_id': src_ou_partner_id.id,
            'company_id': dest_ou.company_id.id,
    ...
    def action_invoice_cancel(self):
        for invoice in self:
            ...
            if dest_ou and not invoice.auto_generated:
                for inter_invoice in self.sudo().search(
                    ...
                    inter_invoice.write({
                        'origin': _('%s - Canceled Invoice: %s') % (
                            invoice.operating_unit_id.name, invoice.number)
                    })
