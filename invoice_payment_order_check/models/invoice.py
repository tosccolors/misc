...
class AccountInvoice(models.Model):
    ...
    @api.depends('line_ids.payment_line_ids')
    def _compute_payment_order(self):
        payment_order_line = self.env['account.payment.line']
        for inv in self.filtered(lambda n: n.move_type == 'in_invoice'):
            pol = payment_order_line.search([('move_line_id', 'in', inv.line_ids.ids)])
