...
class account_move(orm.Model):
    ...
    def _move_reversal(self, cr, uid, move, reversal_date,
        ...
        reversal_period = period_obj.browse(cr,uid, reversal_period_id)
        if not reversal_journal_id:
            reversal_journal_id = move.journal_id.id

        [rev_journal_id] = self.pool.get('account.journal').browse(cr, uid, [
                reversal_journal_id])
        if rev_journal_id.company_id != move.company_id:
            raise Warning("Wrong company Journal is %s but we have %s" % (
                reversal_journal_id.company_id.name, move.company_id.name))
        if reversal_period.company_id != move.company_id:
        ...
        for reversal_move_line in reversal_move.line_id:
            ...
            if reconcile and reversal_move_line.account_id.reconcile:
                if not reversal_move_line.partner_id.id:
                    reversal_move_line.partner_id.id = 0
                rec_dict.setdefault(
                    (reversal_move_line.account_id.id, reversal_move_line.partner_id.id),
                    [])
                rec_dict[(reversal_move_line.account_id.id, reversal_move_line.partner_id.id)]\
                    += [reversal_move_line.id]
        ...
        if reconcile:
            for mline in move.line_id:
                if mline.account_id.reconcile:
                    if not mline.partner_id.id:
                        mline.partner_id.id = 0
                    rec_dict[(mline.account_id.id, mline.partner_id.id)] += [mline.id]
            for to_rec_move_lines in rec_dict.itervalues():
                move_line_obj.reconcile(cr, uid, to_rec_move_lines)
        return reversal_move_id

    def create_reversals(self, cr, uid, ids, reversal_date,
                         reversal_period_id=False, reversal_journal_id=False,
                         move_prefix=False, move_line_prefix=False, reconcile=False,
                         context=None):
        """
