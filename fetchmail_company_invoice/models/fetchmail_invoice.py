...
class AccountInvoice(models.Model):
    ...
    @api.model
    def get_partner_from_mail(self, email_address, company_id, force_supplier=False):
        ...
        for args in args_list:
            related_partners = partner_model.search(args, limit=1)
            if related_partners:
                return related_partners
        return []

    @api.model
    def message_find_partners(self, message, header_fields=['From']):
        ...
        for email_address in tools.email_split(s):
            partner_ids += self.get_partner_from_mail(email_address, company_id)
        return partner_ids


    @api.model
    def _is_partner_supplier(self, partner_id):
        assert partner_id, _(
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        author_id = ('author_id' in msg_dict and msg_dict['author_id'] or False)
        if (author_id and self._is_partner_supplier(author_id)):
        ...
        if not supplier_partner_id:
            ...
            for email_address in tools.email_split(s):
                partner_ids = self.get_partner_from_mail(email_address, company_id, force_supplier=True)
                if partner_ids:
        ...
        if not supplier_partner_id:
            ...
            default_ids = partner_model.search(args)
        ...
        supplier_record = partner_model.browse(supplier_partner_id)
        supplier_company_id = supplier_record.company_id.id
        ...
        if not company_id:
            ...
            company_id = base_model.get_object_reference('base', 'main_company')[1]
        ...
        supplier = partner_model.browse(supplier_partner_id)
        ...
        return super(AccountInvoice, self).message_new(msg_dict, custom_values=custom_values)
