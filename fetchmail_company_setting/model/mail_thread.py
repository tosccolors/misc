...
#             fetchmail_server_id)
#         return fs_record['company_id'] and fs_record['company_id'][0] or False # might be False
#
...
class MailThread(models.AbstractModel):
    ...
    @api.cr_uid_context
    def _get_force_company(self, cr, uid, context=None):
        '''Get company, if any, set on fetchmail server configuration.'''
    ...
    def _message_find_partners(self, message, header_fields=['From']):
        ...
        if (not 'force_company' in context
        or not context['force_company']):
            return super(MailThread, self)._message_find_partners(message, header_fields=header_fields)
    ...
    def message_process(self, cr, uid, model, message, custom_values=None,
        ...
        return super(MailThread, self).message_process(cr, uid, model, message, custom_values,
                        save_original, strip_attachments,
                        thread_id, context=local_context)
